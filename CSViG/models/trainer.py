...
from models.losses import get_alpha, softmax_helper, FocalLoss, mIoULoss, mmIoULoss, cd_loss
...
class CDTrainer():
    ...
    def __init__(self, args, dataloaders):
        ...
        input_data_A = torch.randn(1, 3, 256, 256).to(device=self.device)
        input_data_B = torch.randn(1, 3, 256, 256).to(device=self.device)
        flops, params = profile(self.net_G, (input_data_A, input_data_B))
        print("Total network parameters: \n" + str(params))
        print("Total network flops: \n" + str(flops))
        print(self.device)
        ...
        self.logger.write("\n\nParameters: %s \t\t" % count_parameters(self.net_G))
        self.logger.write("Flops: %s\n" % (flops))
        # define timer
        self.timer = Timer()
        ...
        if args.loss == 'ce':
            ...
            self._pxl_loss = F.binary_cross_entropy
        elif args.loss == 'cd_loss':
            self._pxl_loss = cd_loss
        elif args.loss == 'fl':
